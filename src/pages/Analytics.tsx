    ...
    <div className={`rounded-2xl border p-4 ${isDark ? 'bg-[#141418] border-[#1f1f23]' : 'bg-white border-[#e5e7eb]'}`}>
      ...
      <div className="flex flex-col gap-2">
        {items.map((item, i) => (
          <div
            key={item.id}
            className={`flex items-start gap-2 px-2.5 py-2 rounded-lg ${isDark ? 'bg-[#1a1a1e]' : 'bg-gray-50'}`}
          >
            <span className={`text-[10px] font-black mt-0.5 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>#{i + 1}</span>
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-[10px] font-bold leading-tight ${isDark ? 'text-white' : 'text-[#111827]'}`}>{item.name}</p>
              <p className="text-[10px] font-black text-[#22c55e] mt-0.5">₱{item.price}</p>
            </div>
          </div>
        ))}
      </div>
